    ...
    <script>
        document.addEventListener('livewire:init', () => {
            Livewire.on('printTicket', event => { 
                if(event.url) window.open(event.url, 'Ticket', 'width=400,height=600').focus(); 
            });
            ...
            Livewire.on('submit-cardnet-form', event => {
                ...
                if(!data || !data.url) {
                    console.error('Datos incompletos para Cardnet', data);
                    alert('Error de configuración en pasarela.');
                    return;
                }
                
                form.action = data.url;
                form.innerHTML = ''; // Limpiar campos previos
                ...
                console.log('Redirigiendo a Cardnet:', data.url);
                form.submit();
            });
