<div class="min-h-screen bg-gray-50/50 pb-12">
    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Gestiona tus pagos y descarga comprobantes.</p>
            </div>
    ...
    <div class="mx-auto w-full max-w-[98%] px-4 sm:px-6 lg:px-8 mt-8 space-y-8">
        ...
        <section>
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                <div class="border-b border-gray-100 px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <button wire:click="downloadFinancialReport" class="inline-flex items-center justify-center px-4 py-2 border border-gray-200 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                        <svg class="h-4 w-4 mr-2 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                        Descargar Reporte
                    </button>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse($paymentHistory as $payment)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4"><div class="text-sm font-medium text-gray-900">{{ $payment->paymentConcept->name ?? 'Pago General' }}</div><div class="text-[10px] text-gray-400 font-mono mt-1">ID: {{ $payment->transaction_id ?? '---' }}</div></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $payment->gateway }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset {{ $payment->status === 'Completado' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20' : ($payment->status === 'Pendiente' ? 'bg-amber-50 text-amber-700 ring-amber-600/20' : 'bg-red-50 text-red-700 ring-red-600/20') }}">
                                            {{ $payment->status }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if($payment->status === 'Completado')
                                            <button wire:click="$dispatch('printTicket', { url: '{{ route('finance.ticket', $payment->id) }}' })" class="text-indigo-600 hover:text-indigo-900">Ver Recibo</button>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                    ...
                    <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-200">
                            <div class="bg-white px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                                <h3 class="text-lg font-bold text-gray-900">Realizar Pago</h3>
                            ...
                            <div class="px-6 py-6 bg-white">
                                ...
                                <div x-show="$wire.paymentMethod === 'card'" class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200 text-center">
                                    <p class="text-sm text-gray-600">Serás redirigido a la plataforma segura de <strong>Cardnet</strong>.</p>
                                </div>
                                <div x-show="$wire.paymentMethod === 'transfer'" class="mt-4">
                                    ...
                                    <input type="text" wire:model="transferReference" class="w-full border-gray-300 rounded-lg shadow-sm sm:text-sm py-2.5" placeholder="Ej: 00458822">
                                </div>
        ...
        <div x-data="{ show: false, pdfUrl: '' }" @open-pdf-modal.window="pdfUrl = $event.detail.url; show = true;" x-show="show" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-900/75 backdrop-blur-sm" @click="show = false"></div>
                <div class="inline-block w-full max-w-6xl p-4 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-2xl relative">
                    <button @click="show = false" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            // Lógica para llenar y enviar el form oculto
            Livewire.on('submit-cardnet-form', event => {
                ...
                // Generar inputs hidden dinámicamente
